...
public class BoardDAO {
	...
	private final String updateCnt="UPDATE BOARD SET CNT=CNT+1 WHERE BID=?";
	...
	public boolean update(BoardVO bVO) {
		...
		conn=JDBCUtil.getConnection();
		try {
			if (bVO.getSearchCondition().equals("CNT")) {
				pstmt=conn.prepareStatement(updateCnt);
				pstmt.setInt(1, bVO.getBid());
			}
			else if (bVO.getSearchCondition().equals("UPDATE")) {
